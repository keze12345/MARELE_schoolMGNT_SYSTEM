...
import {
  ...
  Phone, Shield, AlertCircle, Pencil
} from "lucide-react";
...
export default function Staff() {
  ...
  async function handleSave(e) {
    ...
    try {
      ...
      toast.success(editingId ? "Staff member updated!" : `${form.full_name} created successfully!`);
      setShowModal(false);
      setForm(EMPTY_FORM);
      setEditingId(null);
      fetchStaff();
            ...
            <form onSubmit={handleSave} className="p-6 space-y-4">
              ...
              {!editingId && (
                <>
                  ...
                  <div className="p-3 rounded-xl bg-amber-50 border border-amber-100 text-xs text-amber-700">
                    ⚠ Share the email and password securely with the staff member.
                  </div>
                </>
              )}
              ...
              {editingId && (
                <div className="p-3 rounded-xl bg-blue-50 border border-blue-100 text-xs text-blue-700">
                  ℹ Email and password can't be changed here. To reset a password, delete and recreate the account.
                </div>
              )}
